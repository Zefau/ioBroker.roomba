...
class Library
{
	...
	set(node, value)
	{
		...
		this._adapter.getObject(node.node, function(err, obj)
		{
			...
			if (err || node.node === undefined)
				that._adapter.log.error(err !== null ? err.message : 'Error: State not properly defined (' + JSON.stringify(node) + ')!');
	...
	_createNode(node, callback)
	{
		...
		this._adapter.setObject(node.node, {common: Object.assign({role: 'state', type: 'string', 'write': false}, node.common || {}, common), type: 'state', native: node.native || {}}, callback);
	}
